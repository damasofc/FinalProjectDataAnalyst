...
df <- read_csv(structureoOne, col_types = cols(trip_id = col_character()))
...
# merging df & df2 and just getting the rows which tripduration is greater than 1
...
all_data <- all_data %>% 
    mutate(usertype = recode(usertype
                            ,"Subscriber" = "member"
                            ,"Customer" = "casual")) %>%
                            filter(usertype != "Dependent", !is.na(start_time))
# ANALYSIS
all_data %>% 
    group_by(usertype,wday(start_time,label=TRUE)) %>%
